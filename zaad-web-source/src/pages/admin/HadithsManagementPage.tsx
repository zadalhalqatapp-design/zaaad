...
import type { Hadith, Book } from '@/types';
import { BookPlus, Pencil, Trash2, BookOpen } from 'lucide-react';

export function HadithsManagementPage() {
  const { notify } = useToast();
  const { data: booksData, loading: booksLoading } = useAsync(() => api.getBooks().catch(() => [] as Book[]), []);
  const books = booksData || [];
  const [chosenBook, setChosenBook] = useState('');
  // الكتاب المعروض: المختار، وإلا أول كتاب (أو بلا فلتر إن لم توجد كتب — خادم قديم)
  const bookId = chosenBook && books.some((b) => b.id === chosenBook) ? chosenBook : books[0]?.id;
  const currentBook = books.find((b) => b.id === bookId);

  const { data, loading: hadithsLoading, reload } = useAsync(
    () => (booksLoading ? Promise.resolve([] as Hadith[]) : (api.getHadiths(bookId) as Promise<Hadith[]>)),
    [booksLoading, bookId],
  );
  const loading = booksLoading || hadithsLoading;
  ...
  const openAdd = () => {
    setEditing(null);
    const nextNumber = hadiths.reduce((m, h) => Math.max(m, h.number), 0) + 1;
    setForm({ number: String(nextNumber), text: '', explanation: '', youtubeUrl: '', audioUrl: '', pdfUrl: '', category: '', narrator: '' });
    setOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    // نطلب روابط يوتيوب حصرًا للفيديوهات الجديدة أو المُعدَّلة (يُشغَّل مضمَّنًا داخل
    // التطبيق دون مغادرته، بعكس روابط درايف المباشرة). لا نمنع حفظ حديث قديم برابط
    // غير مُعدَّل حتى لا نعطّل تعديل حقول أخرى فيه.
    const newYoutubeUrl = form.youtubeUrl.trim();
    const originalYoutubeUrl = editing?.youtubeUrl?.trim() ?? '';
    if (newYoutubeUrl && newYoutubeUrl !== originalYoutubeUrl && !isYouTubeUrl(newYoutubeUrl)) {
      notify('رابط الفيديو يجب أن يكون من يوتيوب (youtube.com أو youtu.be) ليعمل داخل التطبيق بشكل موثوق.', 'warning');
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        subtitle={`${hadiths.length} عنصرًا${currentBook ? ` في «${currentBook.title}»` : ''}`}
        action={<Button icon={<BookPlus size={18} />} onClick={openAdd}>إضافة عنصر</Button>}
      />

      {books.length > 0 && (
        <div className="mb-4 max-w-sm">
          <Select label="الكتاب" value={bookId || ''} onChange={(e) => setChosenBook(e.target.value)}>
            {books.map((b) => (
              <option key={b.id} value={b.id}>{b.title}{b.status === 'archived' ? ' (مؤرشف)' : ''}</option>
            ))}
          </Select>
        </div>
      )}
      ...
      {hadiths.length === 0 ? (
        <Card><EmptyState icon={<BookOpen size={40} />} title="لا توجد عناصر" description="ابدأ بإضافة محتوى هذا الكتاب." action={<Button onClick={openAdd} icon={<BookPlus size={18} />}>إضافة حديث</Button>} /></Card>
      ) : (
        <Card padded={false}>
          <DataTable<Hadith>
            columns={[
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            label="رابط الفيديو (يوتيوب فقط)"
            value={form.youtubeUrl}
            onChange={(e) => setForm({ ...form, youtubeUrl: e.target.value })}
            dir="ltr"
            hint="رابط يوتيوب فقط (youtube.com أو youtu.be) — يُشغَّل داخل التطبيق مباشرة دون الخروج منه."
          />
          <Input label="رابط الملف الصوتي" value={form.audioUrl} onChange={(e) => setForm({ ...form, audioUrl: e.target.value })} dir="ltr" />
          <Input
            label="رابط صورة الحديث (النص والشرح في صورة واحدة)"
            value={form.pdfUrl}
            onChange={(e) => setForm({ ...form, pdfUrl: e.target.value })}
            dir="ltr"
            hint="ارفع صورة تجمع نص الحديث وشرحه (مثلًا عبر Canva) وضع رابطها هنا — تحل محل ملف PDF القديم."
          />
        </div>
      </Dialog>
