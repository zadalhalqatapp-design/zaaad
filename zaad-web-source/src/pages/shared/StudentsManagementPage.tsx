...
import { StudentBooksDialog } from '@/components/books/StudentBooksDialog';
import { Search, Eye, Ban, RotateCcw, KeyRound, Library } from 'lucide-react';
...
export function StudentsManagementPage() {
  ...
  return (
    <div className="animate-fade-in">
              ...
              key: 'actions', label: 'إجراءات', render: (s) => (
                <div className="flex items-center gap-1">
                  ...
                  {(s.status === 'approved' || s.status === 'suspended') && (
                    <button onClick={() => setBooksTarget(s)} className="p-1.5 rounded-lg hover:bg-primary-50 text-primary-600" title="كتب الطالب"><Library size={16} /></button>
                  )}
                  {s.status !== 'suspended' ? (
      ...
      <StudentBooksDialog student={booksTarget} onClose={() => setBooksTarget(null)} />
