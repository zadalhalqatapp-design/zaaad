    ...
    try {
      ...
      await generateCertificatePDF(
        { 
          ...
          studentGender: user?.gender || cert.studentGender 
        },
        {
          ...
          {certificates.map((cert) => (
            <Card key={cert.id} className="overflow-hidden">
              <div className="bg-gradient-to-l from-primary-600 to-primary-700 text-white p-5 -m-5 mb-4">
                ...
                <h3 className="text-lg font-bold mt-3">{cert.bookTitle || cert.cycleName}</h3>
                {cert.bookTitle && <p className="text-sm text-white/80 mt-0.5">{cert.cycleName}</p>}
              </div>
              <div className="space-y-2 mb-4">
                <Row label="رقم الشهادة" value={cert.certificateNumber} />
                <Row label="تاريخ الإصدار" value={cert.issueDate} />
                {cert.grade && <Row label="التقدير" value={cert.grade} />}
