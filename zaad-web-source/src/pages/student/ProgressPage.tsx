...
export function ProgressPage() {
  ...
  const mediaPct = stats.total ? (mediaCount / total) * 50 : 0;
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        ...
        <Card className="lg:col-span-2">
          <CardHeader title="تفصيل المهارات" subtitle="الحفظ 50% · السماع (صوت أو فيديو) 50%" />
          <div className="space-y-5">
            <ProgressRow icon={<BookMarked size={20} />} label="الحفظ" count={memorizedCount} total={total} pct={memorizedPct * 2} color="primary" />
            <ProgressRow icon={<Headphones size={20} />} label="السماع (صوت أو فيديو)" count={mediaCount} total={total} pct={mediaPct * 2} color="info" />
          </div>
          <p className="mt-4 flex items-center gap-1.5 text-xs text-on-surface-variant">
            <FileText size={14} /> اطّلعت على صورة الحديث في {readCount} من {total} — مادة مساعدة لا تدخل في النسبة.
          </p>
        </Card>
      </div>
