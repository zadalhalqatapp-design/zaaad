...
interface ProgressBarProps {
  ...
  buffered?: number;
  /** أقصى نسبة (0-1) يمكن السحب/التقديم إليها — لمنع تخطي المقاطع غير المُشاهَدة فعليًا. افتراضيًا 1 (بلا قيد). */
  maxRatio?: number;
}

export function ProgressBar({ current, duration, onSeek, buffered = 0, maxRatio = 1 }: ProgressBarProps) {
  const trackRef = useRef<HTMLDivElement>(null);
  const [dragRatio, setDragRatio] = useState<number | null>(null);
  const safeMaxRatio = Math.min(1, maxRatio);

  const ratioFromClientX = useCallback((clientX: number) => {
    const el = trackRef.current;
    if (!el) return 0;
    const rect = el.getBoundingClientRect();
    return Math.min(safeMaxRatio, Math.max(0, (clientX - rect.left) / rect.width));
  }, [safeMaxRatio]);
  ...
  return (
    ...
    >
      ...
      {safeMaxRatio < 1 && (
        <div className="absolute inset-y-0 right-0 rounded-full bg-black/40 pointer-events-none" style={{ width: `${(1 - safeMaxRatio) * 100}%` }} />
      )}
      <div
