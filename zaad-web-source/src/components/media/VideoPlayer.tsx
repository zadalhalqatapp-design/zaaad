...
import { Play, Pause, RotateCcw, RotateCw, Maximize, Minimize, AlertCircle, RefreshCw, CheckCircle } from 'lucide-react';
import { ProgressBar, SpeedMenu, formatTime } from './PlayerControls';
import { isYouTubeUrl, getYouTubeId, isGoogleDriveUrl, getDriveFileId, toStreamableUrl, loadYouTubeApi, type YTPlayerInstance } from '@/lib/media';
...
export interface VideoPlayerProps {
  ...
  /** يظهر فقط لفيديوهات Google Drive: هل سُجِّلت مشاهدتها يدويًا من قبل؟ */
...
export function VideoPlayer(props: VideoPlayerProps) {
  ...
  // فيديوهات Drive: رابط "uc?export=download" الذي نحوّل إليه للتشغيل المباشر يُرجع من
  // درايف صفحة HTML "تعذّر فحص الفيروسات" بدل بثّ الفيديو نفسه للملفات الكبيرة، فيفشل
  // عنصر <video> في تشغيلها. الحل: مشغّل درايف الرسمي نفسه (iframe لرابط /preview) —
  // يعمل بصرف النظر عن حجم الملف. لا واجهة برمجية لقراءة نسبة مشاهدته، فزر يدوي بدلًا.
  if (isGoogleDriveUrl(props.src)) return <DriveVideoPlayer {...props} />;
  return <NativeVideoPlayer {...props} src={toStreamableUrl(props.src)} />;
}

function DriveVideoPlayer({ src, onFlush, alreadyWatched }: VideoPlayerProps) {
  const fileId = getDriveFileId(src);
  const [watched, setWatched] = useState(!!alreadyWatched);
  if (!fileId) return <p className="text-sm text-error-600">رابط فيديو Google Drive غير صالح.</p>;
  return (
    <div className="rounded-xl overflow-hidden bg-black">
      <iframe
        ...
        title="فيديو الحديث"
      />
      <div className="bg-surface p-2.5 flex items-center justify-between gap-2">
        <p className="text-[11px] text-on-surface-variant">
          يُعرَض هذا الفيديو داخل مشغّل Google نفسه؛ لا تتبّع تلقائي للنسبة.
        </p>
        <button
          onClick={() => { if (!watched) { setWatched(true); onFlush(100, 0); } }}
          disabled={watched}
          className={`shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            watched ? 'bg-success-50 text-success-700' : 'bg-primary-600 text-white hover:bg-primary-700'
          }`}
        >
          <CheckCircle size={14} /> {watched ? 'تم تسجيل المشاهدة' : 'شاهدت الفيديو'}
        </button>
      </div>
    </div>
  );
...
function NativeVideoPlayer({ src, initialPosition = 0, onProgress, onFlush }: VideoPlayerProps) {
  ...
  // أقصى موضع وصله التشغيل الطبيعي فعليًا — يُستخدم لمنع تخطي أجزاء لم تُشاهَد بعد
  // عبر السحب/التقديم في خط الزمني (إكمال المشاهدة يجب أن يمر بكل المقطع).
  const [maxReached, setMaxReached] = useState(initialPosition);
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      setMaxReached((m) => Math.max(m, v.currentTime));
  ...
  const skip = (delta: number) => {
    ...
    v.currentTime = delta > 0 ? Math.min(target, maxReached) : target;
  };
  const seek = (time: number) => {
    const v = videoRef.current;
    if (!v) return;
    const target = Math.min(Math.max(0, time), v.duration || 0);
    v.currentTime = Math.min(target, maxReached);
  };
    ...
    <div ref={containerRef} dir="ltr" className="relative bg-black rounded-xl overflow-hidden group">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-3 pt-8 pb-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
        <div className="flex items-center gap-2 mb-1.5">
          <ProgressBar current={current} duration={duration} onSeek={seek} buffered={buffered} maxRatio={duration > 0 ? maxReached / duration : 1} />
        </div>
...
function YouTubeVideoPlayer({ videoId, initialPosition = 0, onProgress, onFlush }: VideoPlayerProps & { videoId: string }) {
  ...
  const skip = (delta: number) => {
    const p = playerRef.current;
    if (!p) return;
    const target = Math.min(Math.max(0, p.getCurrentTime() + delta), p.getDuration());
    p.seekTo(delta > 0 ? Math.min(target, maxReached) : target, true);
  };
  const seek = (time: number) => {
    const p = playerRef.current;
    if (!p) return;
    p.seekTo(Math.min(Math.max(0, time), maxReached), true);
  };
  const changeRate = (r: number) => {
  ...
  return (
    <div ref={containerRef} dir="ltr" className="relative bg-black rounded-xl overflow-hidden group aspect-video">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-3 pt-8 pb-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
        <div className="flex items-center gap-2 mb-1.5">
          <ProgressBar current={current} duration={duration} onSeek={seek} maxRatio={duration > 0 ? maxReached / duration : 1} />
        </div>
        <div className="flex items-center justify-between text-white">
          <div className="flex items-center gap-1">
            <button onClick={() => skip(-10)} className="p-1.5 hover:bg-white/15 rounded-lg" aria-label="تأخير 10 ثوان"><RotateCcw size={18} /></button>
